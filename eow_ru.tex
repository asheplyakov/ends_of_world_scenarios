...
\begin{frame}[plain]
\vspace{-5.4cm}
\centerline{Большой П с точки зрения физики}
\end{frame}
}
...
\begin{itemize}
\item Вызывает массовое вымирание видов
      \begin{itemize}
        \item Столкновения с астероидами
        \item Взрывы сверхновых
        \item Гамма-всплески
      \end{itemize}
\item Навсегда делает Землю непригодной к жизни
      \begin{itemize}
      \item Смещение обитаемой зоны
      \end{itemize}
\item Физически разрушает Землю
      \begin{itemize}
...
\begin{frame}
\frametitle{Тепло и свет в каждый дом}
\only<1>{
\begin{center}
\includegraphics[height=0.82\textheight]{img/chelyabinsk_meteor.jpg}
\end{center}
}
\only<2>{
\begin{columns}[c]
...
\begin{itemize}
\item $\gamma$-лучи разбивают на атомы молекулы воздуха $N_2$ and $O_2$
\item Из атомарного азота и кислорода образуются оксиды азота $NO$, $NO_2$
\item Эти молекулы могут находится в стратосфере годами
\item $NO_2$ хорошо поглощает видимый свет
\end{itemize}
\end{block}

\begin{block}{Повышение уровня солнечного УФ излучения}
\begin{itemize}
\item $NO$ разрушает озон: $NO + O_3 \to NO_2 + O_2$
...
\begin{block}{Гравитация против термоядерного синтеза}
\begin{itemize}
...
\end{itemize}
\end{block}
\begin{block}{Звёзды становятся ярче по мере исчерпания водорода}
\begin{itemize}
...
\item 47 -- 52\% SiO2, 14 --18\% Al2O3, {\bf 6 --12\% CaO}
\end{itemize}
\end{block}
...
\item Концентрация CO2 через 600 млн. лет: 50 ЧНМ (сейчас: 400 ЧНМ)
...
\begin{itemize}
\item Температура поверхности Земли поднимается до 32O K ($47^\circ \mathrm{C}$)
