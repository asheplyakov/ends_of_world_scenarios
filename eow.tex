...
\begin{frame}[plain]
\vspace{-5.4cm}
\Large{\centerline{Ends of the World are coming}}
\end{frame}
}
...
\end{block}
\column{0.5\textwidth}
\begin{block}{Kinetic wheapons}
$E = \frac{m v^2}{2} = \frac{2}{3} \pi \rho R^3 v^2$ \\
$\rho \sim 3000 \: \mathrm{kg/m^3}$ {\small (granite)} \\
$v \sim 30 - 80 \: \mathrm{km/sec}$ {\small (Solar system bodies)} \\
{\Large \color{red} $E \sim 10 \: \mathrm{MT}$} \\
$1 \: \mathrm{MT} = 4.18 \cdot 10^{15} \mathrm{J}$
\end{block}
\begin{block}{Size matters}
$D < 10 \, \mathrm{m}$: vaporizes in the atmosphere \\
$D \geq 20 - 100 \, \mathrm{m}$: hits the surface \\
\end{block}
\end{columns}
}
\end{frame}
...
\begin{frame}
\frametitle{Supernovae explosions}
\only<1>{
\begin{itemize}
...
\end{itemize}
\begin{block}{This will never happen to me}
\begin{itemize}
